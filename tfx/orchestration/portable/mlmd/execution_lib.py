...
def get_artifacts_dict(
  ...
  for key, indexed_artifact_ids in indexed_artifact_ids_dict.items():
    ...
    if indices != list(range(0, len(indices))):
      raise ValueError(
          'Cannot construct artifact ids dict due to missing or duplicate '
          'indices: {}'.format(indexed_artifact_ids_dict))
    artifact_ids_dict[key] = [aid for _, aid in ordered_artifact_ids]

  # Fetch all artifact types and create a map keyed by artifact type name.
  artifact_types = metadata_handler.store.get_artifact_types()
